...
There are several ways to index a data frame, since it is in a tabular (matrix) format, you can index it the same way as a matrix. Here are a couple of examples using the `mtcars` data frame.
...
To import a data set using RStudio, head over to the E
nvironment tab (usually in the upper right hand pane) and click on the Import Dataset button. A pop-up window should look something like below.
